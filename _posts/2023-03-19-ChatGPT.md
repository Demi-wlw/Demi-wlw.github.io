...
After training the model with the objective function above, they adapt the parameters to the supervised target task which refers to supervised fine-tuning_. Assume a labelled dataset $$\mathcal{C}$$, where each instance consists of a sequence of input tokens, $$x^1,\dots, x^m$$, along with a label $$y$$. The inputs are passed through the pre-trained model to obtain the final transformer block's activation $$h_l^m$$, which is then fed into an added linear output layer with parameters $$W_y$$ to predict $$y$$:
{: .text-justify}
